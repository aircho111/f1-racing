...
//	public boolean isLogger = true; // 테스트 로그용 출력

	public DrivingCmd controlDriving(double[] driveArray, double[] aicarArray, double[] trackArray,
			double[] damageArray, int[] rankArray, int trackCurveType, double[] trackAngleArray,
			double[] trackDistArray, double trackCurrentAngle) {
...
//		int opponent_rank = rankArray[DrivingInterface.rank_opponent_rank];
		...
		forward_track_info = this.getForwardTrackInfo(speed, track_forward_angles, track_forward_dists,
				track_current_angle, toStart, angle, track_dist_straight, track_curve_type);
...
//		}
		...
		// corr_toMiddle = this.getCorrToMiddle(dist_cars, toMiddle, speed,
		// angle, track_width, track_dist_straight, track_curve_type);
		corr_route[0] = -1.0;
		corr_route[1] = 0.0;
		corr_route[2] = 100.0;
		corr_route = this.getCorrToMiddle(dist_cars, toMiddle, speed, angle, track_width, track_dist_straight,
				track_curve_type);
		...
		if (emer_turn_yn > 1.0) {
			...
			user_change_dist = this.getChangeDist(dist_cars, toMiddle, user_change_dist[0], speed, track_width,
					track_dist_straight, track_curve_type);
		...
		// 브레이크, 엑셀 제어 <-- 자체함수(임시로 전방 커브 10M 전에 차량의 속도가 110K 이상인 경우
		// 브레이킹...커브에서는 커브경사도별로 구분함)
		...
		// streer_coeff = this.getSteerCoeff2(track_current_angle,
		// track_forward_angles, track_curve_type);
...
//		}
	...
	public static void main(String[] args) {
		...
		if (result == 0) {
			boolean doLoop = true;
			while (doLoop) {
				result = driving.ReadSharedMemory(driveArray, aicarArray, trackArray, damageArray, rankArray,
						trackCurveType, trackAngleArray, trackDistArray, trackCurrentAngle);
				switch (result) {
				case 0:
					DrivingCmd cmd = controller.controlDriving(driveArray, aicarArray, trackArray, damageArray,
							rankArray, trackCurveType[0], trackAngleArray, trackDistArray, trackCurrentAngle[0]);
	...
	private double[] getForwardTrackInfo(double curr_speed, double[] curr_track_forward_angles, double[] curr_track_forward_dists, double curr_track_current_angle, double curr_toStart, double curr_angle, double curr_track_dist_straight, double curr_track_curve_type){
		...
		forward_track_info[1] =  0; // 커브각도

		double user_forward_dists = 0;
		
		double track_chage_angle  = 0;
		double user_change_angle  = 0;
//		long start_time = System.nanoTime();

		double track_before_comp_angle = 0;
		double track_before_comp_dists = 0;
		
		int track_before_comp_change_num = 1; 
		int change_point = 0;     // 각도 변경(45도이상)
		int change_num_point = 0; // 변곡점 건수 1개이면 단일 커프, 2개이상이면 S 커브
		for(int i=0;i<20;i++){
			
			if(curr_track_dist_straight > 10) break;
			// 전방 트랙변경 정보가 바뀌는 첫 변곡점, 두번째 변곡점으로 판단
			// 다음트랙정보로 넘어갈 경우 거리가 변경되나 트랙변경이 짧은 경우는 거리로 고려하기 힘듬
			
			/* 상대거리 계산 */
			user_forward_dists = curr_track_forward_dists[i] - curr_toStart;
//			System.out.println("curr_track_dist_straight["+i+"]   = " + curr_track_dist_straight);
//			System.out.println("curr_speed["+i+"]   = " + curr_speed);
			
//			System.out.println("track_forward_dists["+i+"]   = " + user_forward_dists);
//			System.out.println("  track_current_angle     = " + curr_track_current_angle*180/3.14 + " 도");
//			System.out.println("  track_forward_angles["+i+"]  = " + curr_track_forward_angles[i]*180/3.14 + " 도");
						
			track_chage_angle = curr_track_current_angle - curr_track_forward_angles[i];
				
//			System.out.println("  track_chage_angle["+i+"]      = " + ( track_chage_angle*180/3.14) + " 도");
...
//			System.out.println("track_forward_angles 파이["+i+"] = " + curr_track_forward_angles[i]);
			...
			if(i > 0){
				track_before_comp_angle = curr_track_forward_angles[i] - curr_track_forward_angles[i-1];
				track_before_comp_dists = curr_track_forward_dists[i] - curr_track_forward_dists[i-1];
				
				if( track_before_comp_angle < 0 ){
					if(track_before_comp_change_num > 0){
						change_num_point++;
					}
					track_before_comp_change_num = -1;
				}else if( track_before_comp_angle > 0 ){
					if( track_before_comp_change_num < 0 ) {
						change_num_point++;
					}
					track_before_comp_change_num = 1;
				...
				if(curr_track_forward_angles[i] < 0){
					track_before_comp_change_num = -1;
				}
			}
			
			if(track_before_comp_dists < 7){ // 같은 트랙은 5m차로 제공되는것으로 보임
//				if(isLogger){
//					System.out.println("track_forward_dists["+i+"]   = " + user_forward_dists);
//					System.out.println("  track_current_angle     = " + curr_track_current_angle*180/3.14 + " 도");
//					System.out.println("  track_forward_angles["+i+"]  = " + curr_track_forward_angles[i]*180/3.14 + " 도"); 
...
//					System.out.println("track_current_angle 파이["+i+"]  = " + ( curr_track_current_angle ));
//					System.out.println("track_forward_angles 파이["+i+"] = " + curr_track_forward_angles[i]);
//					System.out.println("track_before_comp_angle 파이["+i+"] = " + track_before_comp_angle);
//					System.out.println();
//				}
				
				if(Math.abs(track_before_comp_angle) > 0.13){ // 45(0.705)도 이상 발생시(급커브 판단기준)
					// 0.13 90도 커브
					change_point++;
				}
			}
			
			if(i == 0) // 전방 1m 이내인 경우(트랙이 다수 변경되는 점)
			{
				if(user_forward_dists < 3){
					user_change_angle = curr_angle + track_chage_angle;
					forward_track_info[0] = user_change_angle;
//					if(isLogger){
//						System.out.println();
//						System.out.println("user_change_angle 파이["+i+"]    = " + user_change_angle);				
//						System.out.println();
//					}
				}
			}else{
...
//		if(isLogger){
//			System.out.println("================= change_point cnt       = " + change_point);
//			System.out.println("================= change_num_point cnt   = " + change_num_point);
//		}
//		System.out.println("user_change_angle time = " + (end_time-start_time));
	 ...
	 * @return
	 */
	private double getSteerAngle(double curr_angle, double curr_toMiddle, double curr_track_width,
			double streer_coeff) {
		double steer_angle = 0.0;
...
//		}
		...
		if (track_curve_type == 1) {
			...
			// System.out.println("전방 첫번째 angle : " +
			// Math.abs(track_forward_angles[0]));
			// System.out.println("현재 트랙angle을 뺀 나머지와 전방 첫번째 angle의 합 : " +
			// currRemainAndForwardAngleSum);
			// System.out.println("현재 트랙angle을 뺀 나머지와 전방 첫번째 angle의 합을 뺀 나머지 : "
			// + realRemain);
			// }
		}
		// 좌회전인 경우
			...
			// System.out.println("전방 첫번째 angle : " +
			// Math.abs(track_forward_angles[0]));
			// System.out.println("현재 트랙angle와 전방 첫번째 angle을 뺀 나머지의 합 : " +
			// currRemainAndForwardAngleSum);
			// System.out.println("현재 트랙angle와 전방 첫번째 angle을 뺀 나머지의 합을 뺀 나머지 : "
			// + realRemain);
			// }
		}
...
//		}
	 ...
	 * @return
	 */
	private double[] getCorrToMiddle(double[] curr_aicars, double curr_toMiddle, double curr_speed, double curr_angle,
			double curr_track_width, double curr_track_dist_straight, double curr_track_curve_type) {
		double corr_toMiddle = 0.0;
...
//			}
			...
			if (tmp_c_ai_cnt > 0) {
				...
				corr_toMiddle = 2.7 + this.getAiSideDist(curr_toMiddle, curr_aicars[tmp_c_ai[0]]); // 전방
																									// 장애물차의
																									// 차폭을
																									// 2.5M라고
																									// 가정
...
//				if(isLogger){
//					System.out.println(
//						"  이동할 거리 : " + corr_toMiddle + ", 트랙까지 거리 : " + (-curr_track_width / 2 + curr_toMiddle));
//				}
				emer_turn_yn = 1.0;
				// 트랙 바깥인 경우
				if ((-curr_track_width / 2 + curr_toMiddle) > corr_toMiddle) {
			...
			if (tmp_c_ai_cnt > 0) {
				...
				if (tmp_left_width > tmp_right_width) {
					...
					if (tmp_left_width > my_car_width) {
						corr_toMiddle = this.getAiSideDist(curr_toMiddle, curr_aicars[tmp_l_ai[0]])
								+ tmp_left_width / 2;

						// 간격이 내 차량이 지나가기에 충분하지 않은 경우 양쪽 차량의 바깥쪽으로 진행(간격이 작은 쪽
						// 우선 체크)
					} else {
						...
						if (curr_aicars[tmp_r_ai[0] - 1] > my_car_length) {
							...
							// 양쪽이 모두 차량으로 막혀 있는 경우 전방 차량이 7M 앞에 올때까지는 현재 진행경로
							// 유지
					...
					if (tmp_right_width > my_car_width) {
						corr_toMiddle = this.getAiSideDist(curr_toMiddle, curr_aicars[tmp_r_ai[0]])
								- tmp_right_width / 2;
				...
				// corr_toMiddle =
				// this.getAiSideDist(curr_toMiddle,curr_aicars[tmp_l_ai[0]]) +
				// this.getAiSideDist(curr_aicars[tmp_l_ai[0],curr_aicars[tmp_r_ai[0]])/2;
				corr_toMiddle = this.getKeepTrackSideDist(curr_toMiddle, curr_track_width);
			}
...
//					}
					...
					if (curr_track_dist_straight > 15.0 && curr_whole_track_dist_straight > 50.0) {
						...
						if (curr_toMiddle < 0) {
							...
							// 트랙 바깥으로 벗어나는 경우 트랙까지만 경로 셋팅...전방에 있는 장애물과 부딪힐 경우도
							// 생각해야함...
							if ((curr_track_width / 2 - 1.5 + curr_toMiddle) < corr_toMiddle) {
								corr_toMiddle = curr_track_width / 2 + curr_toMiddle - 2.0;
							}
		...
		if (tmp_c_ai_cnt > 0) {
			ret_corr_route[2] = curr_aicars[tmp_c_ai[0] - 1]; // 제일 근접한 전방 ai 차량
																// 거리
		}
//		if (isLogger) {
...
//		}
	...
	/**
	 * 현재 진행 경로를 유지하기 위한 값 (내차 기준) 0값을 셋팅하면 현재 경로가 유지되나 계산시 미세보정
	 * 
	 * @param curr_toMiddle
	 * @param curr_track_width
	 * @return
	 ...
	 * @return
	 */
	private double getBestSpeed(double curr_angle, double curr_dist_aicar, double curr_speed,
			double curr_track_dist_straight, double curr_track_curve_type) {
		double user_best_speed = 100;
		...
		user_best_speed = curr_max_speed
				* (1 - Math.exp(-user_c_coeff / curr_max_speed * curr_dist_aicar - user_d_coeff));
//		if (isLogger) {
...
//			}
	 ...
	 * @return
	 */
	private double[] getSpeedCtl2(double curr_speed, double curr_best_speed, double curr_track_dist_straight,
			double track_curve_level) {
		double corr_break = 0.0;
...
//		}
	 ...
	 * @return
	 */
	private double[] getChangeDist(double[] curr_aicars, double curr_toMiddle, double change_toMiddle,
			double curr_speed, double curr_track_width, double curr_track_dist_straight, double curr_track_curve_type) {
...
//		}
		...
		user_change_dist[0] = change_toMiddle; // 변경할 거리 (초기는 최적화된 코스로 셋팅)
		user_change_dist[1] = 100; // 전방 장애물 거리

		// 전방 차량 모두 검증
		double[] cars_dist = new double[2];
		cars_dist[0] = 2; // 차량 실거리
		cars_dist[1] = 2; // 차량 중심거리(좌우 판단 가능)
...
//					}
			...
			cars_dist = this.getCarsDist(curr_toMiddle, change_toMiddle, curr_aicars[2 * i + 1], curr_track_curve_type,
					user_car_width, ai_car_length);

			// 차량 중심거리가 음수 이면 차량이 충돌하는 것으로 판단하여 우회
...
//				}
				if (cars_dist[1] > 0) { // 장애물 차가 우측에 존재(왼쪽으로 기본 이동)
					change_toMiddle = change_toMiddle + (Math.abs(cars_dist[0]) + user_car_width);
				} else if (cars_dist[1] < 0) { // 장애물 차가 좌측에 존재(오른쪽으로 기본 이동)
					change_toMiddle = change_toMiddle - (Math.abs(cars_dist[0]) + user_car_width);
				} else {
...
//		}
	 ...
	 * @return
	 */
	private double[] getCarsDist(double curr_toMiddle, double user_change_toMiddle, double ai_car_toMiddle,
			double curr_track_curve_type, double user_car_width, double ai_car_length) {
		double[] cars_dist = new double[2];
		cars_dist[0] = 0; // 차량간 실거리
		cars_dist[1] = 0; // 차량 중심거리(좌우 판단 가능)

		double cars_middle_dist = curr_toMiddle - ai_car_toMiddle; // 차량과의 중심 거리
		cars_dist[1] = cars_middle_dist;
//		if (isLogger) {
//			System.out.println("cars_middle_dist(중심거리) = " + cars_middle_dist);
//		}

		// 차량간 실거리 (차폭을 고려하여 계산)
		cars_dist[0] = Math.abs(cars_middle_dist) - user_car_width / 2 - ai_car_length / 2;
//		if (isLogger) {
//			System.out.println("cars_dist(실거리)         = " + cars_dist[0]);
//		}
